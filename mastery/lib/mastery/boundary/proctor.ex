defmodule Mastery.Boundary.Proctor do
  ...
  def notify_start(%{notify_pid: nil}), do: nil

  def notify_start(quiz) do
    send(quiz.notify_pid, {:started, quiz.fields.title})
  end

  def notify_stopped(nil, _title), do: nil
  def notify_stopped(pid, title), do: send(pid, {:stopped, title})
end
